...
describe('api', function () {
  ...
  it('should return status when requested', (done) => {
    ...
    const actions = {
      apiActions: {
        getTransforms$: never(),        getStatus$: of(undefined),        captureScreen$: never()
      }
    }
  ...
  if (typeof (document) !== 'undefined') {
    it('should capture screen when requested', (done) => {
      ...
      const actions = {
        apiActions: {
          getTransforms$: never(),          getStatus$: never(),          captureScreen$: of({
            request: 'captureScreen',            element: document.createElement('div')
          })
        }
      }
